...
/* Meta controls ride with the prompt bar: when it's up (no data-prompt-hidden
   ...
   while the canvas requirements panel is open (data-canvas-reqs-open) — that's
   a focused mode with every meta control out of the way. visibility (not just
   opacity) so a hidden button truly leaves the rendered tree. The
   data-needs-repair rule below still forces Repair visible regardless — an
   errored component must always be fixable. */
body:not([data-prompt-hidden="true"]):not([data-canvas-reqs-open="true"]) .component-chrome {
    opacity: 1;
    visibility: visible;
