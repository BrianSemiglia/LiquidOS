...
.component-content, .component-requirements {
    grid-area: stack;
    min-width: 0;
}
...
.component-requirements textarea {
    width: 100%;
    min-height: 10rem;
    resize: vertical;
    border: 1px solid transparent;
...
.component-requirements textarea:focus {
    outline: none;
...
.component-requirements button,
.component-chrome-button {
    border: 1px solid transparent;
    border-radius: 6px;
...
.component-requirements button:hover:not(:disabled),
.component-chrome-button:hover:not(:disabled) {
    background: var(--control-hover);
}

/* The Requirements button reads like the sidebar's Cancel button: a
...
.component-requirements button:disabled { opacity: 0.48; }
...
.component-chrome-button:focus-visible {
...
.requirements-overlay .component-frame {
    width: 100%;
...
.requirements-overlay .component-content, .requirements-overlay .component-requirements {
    grid-area: auto;
    min-width: 0;
    min-height: 0;
    height: 100%;
...
.requirements-overlay .component-requirements {
    display: grid;
...
.requirements-overlay .component-requirements textarea {
    min-height: 0;
    height: 100%;
    resize: none;
}
...
class LiquidOSComponent extends HTMLElement {
    ...
    prepareForPersist() {
        ...
        const frame = this.querySelector(':scope > section.item > div.component-frame');
        if (!frame) return;
        const surface = frame.querySelector(':scope > div.component-content > div.surface');
    ...
    connectedCallback() {
        ...
        this.workspacePath = (canvas && this.path && !this.path.startsWith(canvas + '/'))
            ? canvas + '/' + this.path
            : this.path;
        // Stream-surface contract: the agent's lqpatch stream persists into the
        // nearest [data-stream-surface] ancestor's file. A component's is its
        // component.html, serialized as the whole element.
        this.dataset.streamSurface = '';
        // Nested chrome guard: when we're rendering inside the old
        // harness's component frame (.component-frame
        ...
        if (this.closest('.component-frame')) {
            ensureStyle();
            return;
        }
        ensureStyle();
    ...
    streamSurfaceFile() {
        return this.workspacePath ? this.workspacePath + '/component.html' : '';
    }

    streamSurfaceSerialize(clone) {
    ...
    disconnectedCallback() {
        ...
        queueMicrotask(() => {
            ...
            if (this._item) window.liquidos?.detachComponentChrome?.(this._item);
        });
    }
    ...
    //           <button class="component-chrome-button">Repair</button>
    //         </liquidos-callback>
    //         <button class="component-chrome-button" data-requirements-button>Requirements</button>
    //       </div>
    ...
    buildShell() {
        ...
        runtimeRepair.innerHTML = '<button type="button" class="component-chrome-button">Repair</button>';

        const requirementsButton = document.createElement('button');
        requirementsButton.type = 'button';
        requirementsButton.className = 'component-chrome-button';
        requirementsButton.dataset.requirementsButton = 'true';
        requirementsButton.textContent = 'Requirements';
        ...
        const leaf = String(this.path || '').split('/').filter(Boolean).pop() || 'component';
        const prettyName = leaf.replace(/[-_]+/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        requirementsButton.setAttribute('aria-label', 'Edit ' + prettyName + ' requirements');

        chrome.append(runtimeRepair, requirementsButton);

        const front = document.createElement('div');
        front.className = 'component-content';
        ...
        back.className = 'component-requirements';
        back.innerHTML = [
            '<div class="component-requirements-header">',
            '<div>',
            ...
            '</div>',
            '</div>',
            '<div class="component-requirements-actions">',
        ...
        this._frame = frame;
        this._chrome = chrome;
        // Chrome-lifting is a harness service (it owns the body-level overlay);
        // register our frame+chrome so the buttons lift out when a canvas folds
        // the card, the same as any harness-rendered component.
        window.liquidos?.registerComponentChrome?.(item, chrome, frame);
        this._front = front;
        this._back = back;
        this._surface = surface;
        this._runtimeRepair = runtimeRepair;
        this._requirementsButton = requirementsButton;
        ...
        requirementsButton.addEventListener('click', e => {
            e.stopPropagation();
    ...
    openRequirementsOverlay() {
        ...
        if (this._requirementsButton) this._requirementsButton.textContent = 'Close';

        // Refresh per-component Share state on every open — the canvas
        // toggle may have switched between views. Pass the canvas-relative
        ...
        overlay.addEventListener('click', e => {
            ...
            // requirements panel (.component-requirements). The overlay centers a box
            ...
            if (!e.target.closest('.surface, .component-requirements')) this.closeRequirementsOverlay();
        });
    ...
    closeRequirementsOverlay({ skipMoveBack = false } = {}) {
        ...
        this._overlay = null;
        this._placeholder = null;
        if (this._requirementsButton) this._requirementsButton.textContent = 'Requirements';
        if (this._onKey) {
