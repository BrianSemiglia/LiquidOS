...
const reportedFailing = new Set(); // componentPath -> the last thing we wrote was ok:false

// Throttles a component that throws repeatedly, so one broken render doesn't
// become a write storm. It only ever suppresses a repeat of what the file
// already says — see reportRuntimeError.
const DEDUP_MS = 1500;
...
export const reportRuntimeError = async (componentPath, message, stack) => {
    ...
    // Throttle only a failure the file already records. A failure arriving
    // after a clear is news, and dropping it would leave a broken component
    // marked healthy with nothing left to correct it.
    if (reportedFailing.has(componentPath) && now - last < DEDUP_MS) return;
    lastReportAt.set(componentPath, now);
    reportedFailing.add(componentPath);
...
// Called by <liquidos-file script> after a mount() returns cleanly. The
// mount returning IS the event — nothing failed on the way through, so a
// diagnostic left by an earlier run is stale and clears. Lets agents fix a
// throw by editing the source: re-mount runs clean, the Repair button goes.
//
// Deliberately no quiet window. Waiting to see whether an error shows up is
// a deadline standing in for a signal, and it has to be shorter than the
// throttle above, which left a gap where a second failure was cleared and
// then swallowed. An error that lands after this clear reports itself, and
// because that changes what the file says the throttle can't drop it.
export const noteSuccessfulMount = async (componentPath) => {
    if (!componentPath) return;
    if (!reportedFailing.has(componentPath)) {
        // Nothing of ours is outstanding, so only a diagnostic from an earlier
        // page load could be stale. Read before writing, to stay quiet in the
        // healthy case.
        let current = null;
        try {
    ...
    reportedFailing.delete(componentPath);
