...
const fileKey = (el) => {
    const mode = el.hasAttribute('run') ? 'run'
        ...
        : 'render';
    const streamSurface = el.hasAttribute('stream-surface') ? '#stream-surface' : '';
    return mode + streamSurface + '\0' + String(el.getAttribute('path') || '');
};
...
class LiquidOSFile extends HTMLElement {
    ...
    // their (empty) children untouched.) A `stream-surface` render mount is the
    // exception: its children ARE the source being streamed and persisted, so
    // they must survive the persist walk.
    prepareForPersist() {
        if (!this.hasAttribute('run') && !this.hasAttribute('script') && !this.hasAttribute('stream-surface')) {
            this.innerHTML = '';
        }
    }

    // Stream-surface contract (a `stream-surface` render mount): the agent's
    // lqpatch stream persists into this file, serialized as the fragment its
    // children form; the render mode's own morph then re-renders it.
    streamSurfaceFile() {
        return this.hasAttribute('stream-surface') ? this._path : '';
    }
    ...
    connectedCallback() {
        ...
        // A `stream-surface` render mount opts into being a stream target: mark
        // it so the client's stream router persists patches into its file.
        if (this._mode === 'render' && this.hasAttribute('stream-surface')) {
