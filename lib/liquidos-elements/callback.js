...
`
    ...
    // Called by persistHostComponent on a clone of this element. Strip
    // server-driven busy state — those attributes are runtime decorations,
    // not part of the authored source. (See the persistHostComponent
    // walk in index.html.)
    prepareForPersist() {
    ...
    setLoading(loading) {
        // The server is the source of truth for busy state (via the
        // activeLanes SSE broadcast). Always reconcile the DOM here
        // — don't trust `this.loading` to match the live attribute.
        // A morphed-in callback element from a re-render can arrive
        // with data-busy="true" baked into the file while this.loading
        // is still its constructor default of false; skipping on
        // internal-state match would leave that stale attribute (and
        // the pulsing/inert UI) in place forever.
    ...
    handleDeclaredEvent(event) {
        ...
        // the document-level fallback at the end of the bubble chain.
        // Without this, every callback double-dispatches: the targeted
        // handler runs AND /output gets called by the catch-all.
