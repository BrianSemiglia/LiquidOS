...
//   - "Create" opens the name dialog directly (sharing is disabled, so there's
//     no browse step) as a back-stack grid → name modal, with Escape/Cancel
//     stepping back to the grid, not out to the canvas;
//   - picking a canvas card switches to it and closes the grid;
//   - Escape closes the grid (and the Requirements button comes back).
//
// Asserts visible strings (canvas names, the prompt bar's "↑" Send glyph, the
// name dialog's copy, the "Create" card) coming and going — never geometry or flags.
//
// Run it:  node run-probe.mjs probe-canvas-grid
//
...
export default async ({ url, page }) => {
  ...
  // 3. The new-canvas flow is a back-stack: grid → name modal (sharing is off,
  //    so "Create" opens the name dialog directly, with no browse step). Escape
  //    and Cancel step back to the grid, not all the way out to the canvas.
  const NAME_MODAL = 'New Canvas';

  // "Create" opens the name modal directly (grid stays underneath).
  await page.locator('#canvas-grid-new').dispatchEvent('click');
  await onScreen(NAME_MODAL).catch(() => { throw new Error('"Create" did not open the name modal'); });

  // Escape from the name modal steps back to the grid (not out to the canvas).
  ...
  await page.locator('#canvas-grid-overlay').waitFor({ state: 'visible', timeout: 4000 }).catch(() => {
    throw new Error('Escape from the name modal should step back to the grid, not dismiss it');
  });
  console.log('  ok  Escape from the name modal steps back to the grid');

  // Cancel from the name modal also steps back to the grid.
  await page.locator('#canvas-grid-new').dispatchEvent('click');
  ...
  await page.locator('#canvas-grid-overlay').waitFor({ state: 'visible', timeout: 4000 }).catch(() => {
    throw new Error('Cancel from the name modal should step back to the grid, not dismiss it');
  });
  console.log('  ok  Cancel from the name modal steps back to the grid');
